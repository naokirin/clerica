<script lang="ts">
  ...
  async function interpretExifValue(key: string, value: any): Promise<string> {
    if (value === null || value === undefined) return $t("fileDetail.unknown");
    
    switch (key) {
      case 'Orientation':
        const orientationName = await exifApi.getOrientationName(value);
        return orientationName || $t("exif.unknownValue", { value });
        
      case 'ColorSpace':
        const colorSpaceName = await exifApi.getColorSpaceName(value);
        return colorSpaceName || $t("exif.unknownValue", { value });
        
      case 'MeteringMode':
        const meteringModeName = await exifApi.getMeteringModeName(value);
        return meteringModeName || $t("exif.unknownValue", { value });
        
      case 'LightSource':
        const lightSourceName = await exifApi.getLightSourceName(value);
        return lightSourceName || $t("exif.unknownValue", { value });
        
      case 'WhiteBalance':
        const whiteBalanceName = await exifApi.getWhiteBalanceName(value);
        return whiteBalanceName || $t("exif.unknownValue", { value });
        
      case 'SceneCaptureType':
        const sceneCaptureTypeName = await exifApi.getSceneCaptureTypeName(value);
        return sceneCaptureTypeName || $t("exif.unknownValue", { value });
        
      case 'Contrast':
      case 'Saturation':
      case 'Sharpness':
        const enhancementName = await exifApi.getEnhancementName(value);
        return enhancementName || $t("exif.unknownValue", { value });
        
      case 'SubjectDistanceRange':
        const subjectDistanceRangeName = await exifApi.getSubjectDistanceRangeName(value);
        return subjectDistanceRangeName || $t("exif.unknownValue", { value });
        
      case 'Flash':
        return $t(`exif.flashModes.${value}`) || $t("exif.unknownValue", { value });
        
      case 'WhiteBalance':
        return $t(`exif.whiteBalanceModes.${value}`) || $t("exif.unknownValue", { value });
        
      case 'ColorSpace':
        return $t(`exif.colorSpaces.${value}`) || $t("exif.unknownValue", { value });
        
      case 'MeteringMode':
        return $t(`exif.meteringModes.${value}`) || $t("exif.unknownValue", { value });
        
      case 'ExposureMode':
        return $t(`exif.exposureModes.${value}`) || $t("exif.unknownValue", { value });
        
      case 'SceneCaptureType':
        return $t(`exif.sceneTypes.${value}`) || $t("exif.unknownValue", { value });
        
      case 'ExposureProgram':
        return $t(`exif.exposurePrograms.${value}`) || $t("exif.unknownValue", { value });
        
      case 'FocalLength':
        if (Array.isArray(value) && value.length > 0) {
          return `${value[0]} ${$t('exif.units.mm')}`;
        }
        return `${value} ${$t('exif.units.mm')}`;
        
      case 'ExposureTime':
        if (Array.isArray(value) && value.length > 0) {
          const seconds = value[0];
          if (seconds >= 1) {
            return `${seconds} ${$t('exif.units.seconds')}`;
          } else {
            return $t('exif.units.secondsShort', { value: Math.round(1/seconds) });
          }
        }
        return `${value}`;
      ...
      case 'ISOSpeedRatings':
        ...
        return $t('exif.units.iso', { value });
        
      case 'LightSource':
        return $t(`exif.lightSources.${value}`) || $t("exif.unknownValue", { value });
        
      case 'ResolutionUnit':
        return $t(`exif.resolutionUnits.${value}`) || $t("exif.unknownValue", { value });
        
      case 'YCbCrPositioning':
        return $t(`exif.yCbCrPositions.${value}`) || $t("exif.unknownValue", { value });
        
      case 'XResolution':
      case 'YResolution':
        if (Array.isArray(value) && value.length > 0) {
          return `${value[0]} ${$t('exif.units.dpi')}`;
        }
        return `${value} ${$t('exif.units.dpi')}`;
        
      case 'CompressedBitsPerPixel':
        if (Array.isArray(value) && value.length > 0) {
          return `${value[0]} ${$t('exif.units.bitsPerPixel')}`;
        }
        return `${value} ${$t('exif.units.bitsPerPixel')}`;
        
      case 'ExifImageWidth':
      case 'ExifImageHeight':
      case 'PixelXDimension':
      case 'PixelYDimension':
        return `${value} ${$t('exif.units.pixels')}`;
  ...
  async function getExifTagName(tagNumber: number): Promise<string> {
    ...
    return tagName || $t('fileDetail.tagNumber', { number: tagNumber });
  }
  ...
  async function loadFileTags() {
    ...
    try {
      ...
      console.error($t('error.tagsLoadError'), error);
      errorStore.showError($t('fileDetail.tagsLoadError'));
      currentTags = [];
      originalTags = [];
    } finally {
      isLoadingTags = false;
  ...
  async function saveTagsChanges() {
    ...
    try {
      ...
      for (const tagToProcess of tagsToProcess) {
        ...
        if (existingTag) {
          ...
          try {
            ...
            console.error($t('error.tagsSaveError'), error);
            errorStore.showError($t('fileDetail.tagsCreateError'));
          }
        }
      }
      ...
      console.error($t('error.tagsSaveError'), error);
      errorStore.showError($t('fileDetail.tagsSaveError'));
    } finally {
      isSavingTags = false;
    }
  }
    ...
    <div class="modal-content" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="modal-body">
        <div class="file-detail-section">
          ...
          <div class="detail-grid">
            <div class="detail-item">
              <span class="detail-label">{$t("common.fileDetail.filename")}:</span>
            ...
            <div class="detail-item">
              ...
              <span class="detail-value">{file.is_directory ? $t("common.fileDetail.directory") : (file.mime_type || file.file_type || $t("common.fileDetail.unknown"))}</span>
            </div>
          </div>
        </div>
        ...
        <div class="file-detail-section">
          ...
          <div class="detail-grid">
            <div class="detail-item">
              <span class="detail-label">{$t("common.fileDetail.created")}:</span>
              <span class="detail-value">{formatDate(file.created_at)}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">{$t("common.fileDetail.modified")}:</span>
              <span class="detail-value">{formatDate(file.modified_at)}</span>
            </div>
            {#if file.birth_time}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.birthTime")}:</span>
                <span class="detail-value">{formatDate(file.birth_time)}</span>
              </div>
            {/if}
            {#if file.last_accessed}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.lastAccessed")}:</span>
        ...
        <div class="file-detail-section">
          ...
          <div class="detail-grid">
            {#if file.permissions}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.permissions")}:</span>
                <span class="detail-value">{file.permissions}</span>
              </div>
            {/if}
            {#if file.owner_uid !== null}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.ownerUid")}:</span>
                <span class="detail-value">{file.owner_uid}</span>
              </div>
            {/if}
            {#if file.group_gid !== null}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.groupGid")}:</span>
                <span class="detail-value">{file.group_gid}</span>
              </div>
            {/if}
            {#if file.inode !== null}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.inode")}:</span>
                <span class="detail-value">{file.inode}</span>
              </div>
            {/if}
            {#if file.hard_links !== null}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.hardLinks")}:</span>
                <span class="detail-value">{file.hard_links}</span>
              </div>
            {/if}
            {#if file.device_id !== null}
              <div class="detail-item">
                <span class="detail-label">{$t("common.fileDetail.deviceId")}:</span>
        ...
        <div class="file-detail-section">
          ...
          {:else}
            <TagInput 
              ...
              placeholder={$t('fileDetail.tagsInputPlaceholder')}
            />
          {/if}
        </div>
        ...
        {#if file.metadata}
          <div class="file-detail-section">
            <h4>{$t('fileDetail.fileMetadata')}</h4>
            <div class="metadata-section">
              {#if (() => {
                ...
                {#each Object.entries(JSON.parse(file.metadata)) as [category, data]}
                  <div class="metadata-category">
                    <h5>{(() => {
                      switch(category) {
                        case 'exif': return $t('fileDetail.exifInfo');
                        case 'audio': return $t('fileDetail.audioInfo');
                        default: return $t('fileDetail.categoryInfo', { category: category.toUpperCase() });
                      }
                    })()} </h5>
                    ...
                    {#if category === 'audio'}
                      ...
                      <div class="detail-grid">
                        ...
                        {#if data.duration !== undefined}
                          <div class="detail-item">
                            <span class="detail-label">{$t('fileDetail.duration')}:</span>
                        ...
                        {#if data.bitrate !== undefined && data.bitrate > 0}
                          <div class="detail-item">
                            <span class="detail-label">{$t('fileDetail.bitrate')}:</span>
                            <span class="detail-value">{data.bitrate} {$t('exif.units.kbps')}</span>
                          </div>
                        {/if}
                        {#if data.sample_rate !== undefined && data.sample_rate > 0}
                          <div class="detail-item">
                            <span class="detail-label">{$t('fileDetail.sampleRate')}:</span>
                            <span class="detail-value">{data.sample_rate} {$t('exif.units.hz')}</span>
                          </div>
                        {/if}
                        {#if data.channels !== undefined && data.channels > 0}
                          <div class="detail-item">
                            <span class="detail-label">{$t('fileDetail.channels')}:</span>
                            <span class="detail-value">{data.channels === 1 ? $t('fileDetail.mono') : data.channels === 2 ? $t('fileDetail.stereo') : $t('fileDetail.channels_count', { count: data.channels })}</span>
                          </div>
                        {/if}
                        ...
                        {#if data.tags && typeof data.tags === 'object'}
                          {#if data.tags.title}
                            <div class="detail-item">
                              <span class="detail-label">{$t('fileDetail.title')}:</span>
                              <span class="detail-value">{data.tags.title}</span>
                            </div>
                          {/if}
                          {#if data.tags.artist}
                            <div class="detail-item">
                              <span class="detail-label">{$t('fileDetail.artist')}:</span>
                              <span class="detail-value">{data.tags.artist}</span>
                            </div>
                          {/if}
                          {#if data.tags.album}
                            <div class="detail-item">
                              <span class="detail-label">{$t('fileDetail.album')}:</span>
                              <span class="detail-value">{data.tags.album}</span>
                            </div>
                          {/if}
                          {#if data.tags.year}
                            <div class="detail-item">
                              <span class="detail-label">{$t('fileDetail.year')}:</span>
                              <span class="detail-value">{data.tags.year}</span>
                            </div>
                          {/if}
                          {#if data.tags.genre}
                            <div class="detail-item">
                              <span class="detail-label">{$t('fileDetail.genre')}:</span>
                              <span class="detail-value">{data.tags.genre}</span>
                            </div>
                          {/if}
                          {#if data.tags.track}
                            <div class="detail-item">
                              <span class="detail-label">{$t('fileDetail.track')}:</span>
                        ...
                        {#each Object.entries(data) as [key, value]}
                          {#snippet metadataItem()}
                            {#await Promise.all([ 
                              ...
                              <div class="detail-item">
                                <span class="detail-label">{$t('fileDetail.loading')}</span>
                                <span class="detail-value">{$t('fileDetail.loading')}</span>
                              </div>
                            ...
                            {:catch error}
                              <div class="detail-item">
                                ...
                                <span class="detail-value">{$t('fileDetail.errorMessage', { message: error.message })}</span>
                              </div>
                            {/await}
                          {/snippet}
                          {@render metadataItem()}
              ...
              {:else}
                <div class="error-message">
                  {$t('fileDetail.metadataParseError')}
                </div>
              {/if}
            </div>
          </div>
