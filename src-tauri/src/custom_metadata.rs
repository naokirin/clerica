...
pub async fn create_custom_metadata_key(
    ...
    let db = Database;
    if let Ok(Some(_)) = db.get_custom_metadata_key_by_name(shelf_manager.get_settings_pool(), &request.name).await {
    ...
    match db.create_custom_metadata_key(shelf_manager.get_settings_pool(), &key).await {
...
pub async fn get_custom_metadata_keys(
    ...
    let db = Database;
    match db.get_all_custom_metadata_keys(shelf_manager.get_settings_pool()).await {
        Ok(keys) => Ok(keys),
...
pub async fn update_custom_metadata_key(
    ...
    // 既存のキーを取得してnameを保持
    let keys = match db.get_all_custom_metadata_keys(shelf_manager.get_settings_pool()).await {
        Ok(keys) => keys,
    ...
    match db.update_custom_metadata_key(shelf_manager.get_settings_pool(), &updated_key).await {
        Ok(key) => Ok(key),
...
pub async fn delete_custom_metadata_key(
    ...
    let db = Database;
    match db.delete_custom_metadata_key(shelf_manager.get_settings_pool(), &shelf_manager.get_active_data_pool().map_err(|e| e.to_string())?, &key_id).await {
        Ok(_) => Ok(()),
...
pub async fn get_custom_metadata_key_by_name(
    ...
    let db = Database;
    match db.get_custom_metadata_key_by_name(shelf_manager.get_settings_pool(), &name).await {
        Ok(key) => Ok(key),
        Err(e) => Err(format!("カスタムメタデータキーの取得に失敗しました: {e}")),
    }
}
...
pub async fn set_custom_metadata_value(
    ...
    let db = Database;
    match db.set_custom_metadata_value(&shelf_manager.get_active_data_pool().map_err(|e| e.to_string())?, shelf_manager.get_settings_pool(), &request.file_id, &request.key_id, request.value).await {
        Ok(value) => Ok(value),
...
pub async fn get_custom_metadata_values_by_file(
    ...
    let db = Database;
    match db.get_custom_metadata_values_by_file(&shelf_manager.get_active_data_pool().map_err(|e| e.to_string())?, &file_id).await {
        Ok(values) => Ok(values),
...
pub async fn get_custom_metadata_value(
    ...
    let db = Database;
    match db.get_custom_metadata_value(&shelf_manager.get_active_data_pool().map_err(|e| e.to_string())?, &file_id, &key_id).await {
        Ok(value) => Ok(value),
        Err(e) => Err(format!("カスタムメタデータ値の取得に失敗しました: {e}")),
    }
}
...
pub async fn delete_custom_metadata_value(
    ...
    let db = Database;
    match db.delete_custom_metadata_value(&shelf_manager.get_active_data_pool().map_err(|e| e.to_string())?, &file_id, &key_id).await {
        Ok(_) => Ok(()),
